...
class MCModelBase(typing.Generic[_ModelType]):
    ...
    This base class is essentially a declarative description of a farc.models
    model with a :meth:`.build_model` method to generate an appropriate
    ``farc.models` model instance on demand.
    ...
    def build_model(self, size: int) -> _ModelType:
        """
        Turn this MCModelBase subclass into a farc.models Model instance
...
def _build_mc_model(model: _ModelType) -> typing.Type[MCModelBase[_ModelType]]:
    """
    Generate a new MCModelBase subclass for the given farc.models model.
    ...
    for field in dataclasses.fields(model):
        ...
        if field.type is farc.models._VectorisedFloat:  # noqa
        ...
        if getattr(field_type, '__origin__', None) in [typing.Union, typing.Tuple]:
            ...
            if new_field.type == typing.Tuple[farc.models._VentilationBase, ...]:
                VB = getattr(sys.modules[__name__], "_VentilationBase")
                field_type = typing.Tuple[typing.Union[farc.models._VentilationBase, VB], ...]
            elif new_field.type == typing.Tuple[farc.models._ExpirationBase, ...]:
                EB = getattr(sys.modules[__name__], "_ExpirationBase")
                field_type = typing.Tuple[typing.Union[farc.models._ExpirationBase, EB], ...]
            else:
                # Check that we don't need to do anything with this type.
                for item in new_field.type.__args__:
                    if getattr(item, '__module__', None) == 'farc.models':
                        raise ValueError(
                            f"unsupported type annotation transformation required for {new_field.type}")
        elif field_type.__module__ == 'farc.models':
    ...
    bases = []
    # Update the inheritance/based to use the new MC classes, rather than the farc.models ones.
