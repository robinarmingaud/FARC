...
function draw_alternative_scenarios_plot(
    concentration_plot_svg_id,
    alternative_plot_svg_id,
    times,
    ...
    for (scenario in alternative_scenarios) {
        ...
        scenario_values = alternative_scenarios[scenario].values
        colors.push( alternative_scenarios[scenario].color)
        highest_value = Math.max(highest_value, Math.max(...scenario_values))

        var data = []
        times.map((time, index) => data.push({ 'time': time,
            'hour': new Date().setHours(Math.trunc(time), (time - Math.trunc(time)) * 60), 'value': scenario_values[index] }))
    ...
    var yRange = d3.scaleLinear().domain([0., highest_value]);
    ...
    // Tooltip.
    var focus = {}, tooltip_rect = {}, tooltip_time = {}, tooltip_value = {}, toolBox = {};
    for (const [scenario_name, data] of Object.entries(data_for_scenarios)) {
        ...
        tooltip_value[scenario_name] = focus[scenario_name].append('text')
    ...
    function redraw() {
        ...
        for (const [scenario_name, data] of Object.entries(data_for_scenarios)) {
            var scenario_index = Object.keys(data_for_scenarios).indexOf(scenario_name)
            // Lines.
            lineFuncs[scenario_name].defined(d => !isNaN(d.value))
                .x(d => xTimeRange(d.time))
                .y(d => yRange(d.value));
    ...
    function mousemove() {
        for (const [scenario_name, data] of Object.entries(data_for_scenarios)) {
            if (d3.pointer(event)[0] < graph_width / 2) {
                ...
                tooltip_value[scenario_name].attr('x', 18);
            }
            else {
                tooltip_rect[scenario_name].attr('x', -90)
                tooltip_time[scenario_name].attr('x', -82)
                tooltip_value[scenario_name].attr('x', -82)
            }
            var x0 = xRange.invert(d3.pointer(event, this)[0]),
            ...
            focus[scenario_name].attr('transform', 'translate(' + xRange(d.hour) + ',' + yRange(d.value) + ')');
            focus[scenario_name].select('#tooltip-time').text('x = ' + time_format(d.hour));
            focus[scenario_name].select('#tooltip-value').text('y = ' + d.value.toFixed(2));
        }
    }
