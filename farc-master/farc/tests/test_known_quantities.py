...
import farc.models as models
import farc.data as data
...
@pytest.fixture
def baseline_periodic_biov():
    return models.BIOVFilter(
        active=models.PeriodicInterval(period=120, duration=15),
        q_air_mech=514.74,
    )
...
def test_periodic_biov(baseline_periodic_biov, baseline_room):
    # Interesting transition times for a period of 120 and duration of 15.
    ts = [0, 14 / 60, 15 / 60, 16 / 60, 119 / 60, 120 / 60, 121 / 60, 239 / 60, 240 / 60]
    rate = 514.74 / 75
    aes = [baseline_periodic_biov.air_exchange(baseline_room, t) for t in ts]
    answers = [0, rate, rate, 0, 0, 0, rate, 0, 0]
    npt.assert_allclose(aes, answers, rtol=1e-5)
...
def test_multiple_ventilation_BIOV_window(baseline_periodic_biov, time, expected_value):
    ...
    vent = models.MultipleVentilation([window, baseline_periodic_biov])
    npt.assert_allclose(vent.air_exchange(room,time), expected_value, rtol=1e-5)


def test_multiple_ventilation_BIOV_window_transitions(baseline_periodic_biov):
    tempOutside = models.PiecewiseConstant((0., 1., 2.5),(273.15, 283.15))
    tempInside = models.PiecewiseConstant((0., 24.),(293.15,))
    window = models.SlidingWindow(active=models.SpecificInterval([(1 / 60, 24.)]),
                inside_temp=tempInside,outside_temp=tempOutside,
...
def test_multiple_ventilation_BIOV_HVAC_AirChange(volume, expected_value):
    room = models.Room(volume=volume)
    biov = models.BIOVFilter(
    ...
    vent = models.MultipleVentilation([biov, hvac, airchange])
