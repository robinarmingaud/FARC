...
def test_infected_population_vectorisation(override_params):
    ...
    office_hours = farc.models.SpecificInterval(present_times=[(8, 17)])
    infected = farc.models.InfectedPopulation(
            number=1,
            presence=office_hours,
            mask=farc.models.Mask(
                factor_exhale=0.95,
                η_inhale=0.3,
            ),
            activity=farc.models.Activity(
                0.51,
                defaults['exhalation_rate'],
            ),
            virus=farc.models.Virus(
                viral_load_in_sputum=defaults['viral_load_in_sputum'],
                infectious_dose=50.,
                viable_to_RNA_ratio = 0.5,
                transmissibility_factor=1.0,
            ),
            expiration=farc.models._ExpirationBase.types['Breathing'],
            host_immunity=0.,
    )
