...
import farc.apps.calculator
from farc.apps.calculator.report_generator import generate_permalink

_TIMEOUT = 20.

@pytest.fixture
def app():
    return farc.apps.calculator.make_app()
...
class TestCernApp(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        cern_theme = Path(farc.apps.calculator.__file__).parent.parent / 'themes' / 'cern'
        return farc.apps.calculator.make_app(theme_dir=cern_theme)
...
class TestOpenApp(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        return farc.apps.calculator.make_app(calculator_prefix="/mycalc")
...
class TestError500(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        class ProcessingErrorPage(farc.apps.calculator.BaseRequestHandler):
            def get(self):
                raise ValueError('some unexpected error')
        app = farc.apps.calculator.make_app()
        page = [
