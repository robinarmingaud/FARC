...
:mod:`farc.monte_carlo` module is a good example of doing this - that module uses
...
``farc.monte_carlo.ConcentrationModel`` implementation simply samples all of those
...
class ExposureModel:
    ...
    def deposited_exposure_between_bounds(self, time1: float, time2: float) -> _VectorisedFloat:
        ...
        return (dep_exposure_integrated * emission_rate_per_aerosol *
                f_inf * self.exposed.activity.inhalation_rate *
                (1 - self.exposed.mask.inhale_efficiency()*self.exposed.mask_wear_ratio))
