...
class _VentilationBase:
    ...
    WindowOpening for window based air exchange, and BIOVFilter, for
...
@dataclass(frozen=True)
class BIOVFilter(Ventilation):
    #: The interval in which the BIOV filter is operating.
    active: Interval

    #: The rate at which the BIOV exchanges air (when switched on)
    # in m^3/h
    q_air_mech: _VectorisedFloat

    def air_exchange(self, room: Room, time: float) -> _VectorisedFloat:
        # If the BIOV is off, no air is being exchanged.
        if not self.active.triggered(time):
            return 0.
        # Reminder, no dependence on time in the resulting calculation.
...
class Expiration(_ExpirationBase):
    ...
    def aerosols(self, mask: Mask, mask_wear_ratio: float = 1.0):
        ...
        # OPE : factor in mask_wear_ratio in overall mask_efficiency calculation
        mask_efficiency = mask.exhale_efficiency(self.diameter) * mask_wear_ratio
        # final result converted from microns^3/cm3 to mL/cm^3
        return self.cn * (volume(self.diameter) *
                          (1 - mask_efficiency)) * 1e-12


@dataclass(frozen=True)
class MultipleExpiration(_ExpirationBase):  #TODO check if mask_wear_ratio has to be factord in ?
    """
    ...
    def aerosols(self, mask: Mask, mask_wear_ratio: float = 1.0):
        ...
        # OPE : factor in mask_wear_ratio, adding new parameter to method call
        return np.array([
            weight * expiration.aerosols(mask, mask_wear_ratio) / sum(self.weights)
...
class Population:
    ...
    # OPE : factor in mask_wear_ratio, adding new parameter
    # the fraction of time masks are worn (1.0 = always, 0.0 = never)
    mask_wear_ratio: float
...
class InfectedPopulation(_PopulationWithVirus):
    ...
    def aerosols(self):
        ...
        """
        # OPE : factor in mask_wear_ratio, adding new parameter to method call
        return self.expiration.aerosols(self.mask, self.mask_wear_ratio)
...
class ConcentrationModel:
    ...
    previous_concentration: _VectorisedFloat = 0.
    ...
    def _normed_concentration(self, time: float) -> _VectorisedFloat:
        ...
        if time <= self._first_presence_time():
            return self.previous_concentration
...
class ExposureModel:
    ...
    def deposited_exposure_between_bounds(self, time1: float, time2: float) -> _VectorisedFloat:
        """
        The number of virus deposited on the respiratory tract
        ...
        # OPE : factor in mask_wear_ratio in overall mask_efficiency calculation
        mask_efficiency = self.exposed.mask.inhale_efficiency() * self.exposed.mask_wear_ratio
        return (dep_exposure_integrated * emission_rate_per_aerosol *
                f_inf * self.exposed.activity.inhalation_rate *
                (1 - mask_efficiency))

    def cumulative_deposited_exposure(self, time: float) -> _VectorisedFloat:
        """
               The number of virus deposited on the respiratory tract
               deposited at any time.
        """
        cumulative_deposited_exposure = 0.0
        for start, stop in self.exposed.presence.boundaries():
            if time <= start:
                return cumulative_deposited_exposure
            end = min(time, stop)
            cumulative_deposited_exposure += self.deposited_exposure_between_bounds(start, end)
            if time <= stop:
                return cumulative_deposited_exposure
        return cumulative_deposited_exposure
    ...
    def _dose_infection_probability(self, viral_dose: _VectorisedFloat) -> _VectorisedFloat:
        # viral dose (vD)
        ...
        return (1 - np.exp(-((viral_dose * (1 - self.exposed.host_immunity)) / (infectious_dose *
                                                                                self.concentration_model.virus.transmissibility_factor)))) * 100

    def infection_probability(self) -> _VectorisedFloat:
        return self._dose_infection_probability(viral_dose=self.deposited_exposure())

    def cumulative_infection_probability(self, time: float) -> _VectorisedFloat:
        """
        The probability of infection at any given time
        """
        return self._dose_infection_probability(self.cumulative_deposited_exposure(time))
